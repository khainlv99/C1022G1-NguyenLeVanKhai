 ...
 * percentage options are 1), based on the previous and following loaisanpham. This mode
	...
	Chart.controllers.bar = Chart.DatasetController.extend({
		...
		updateElement: function(rectangle, index, reset) {
			...
			rectangle._model = {
				...
				bchitietdonhangkipped: custom.bchitietdonhangkipped ? custom.bchitietdonhangkipped : rectangleOptions.bchitietdonhangkipped,
...
module.exports = Element.extend({
	draw: function() {
		var ctx = this._chart.ctx;
		var vm = this._view;
		var left, right, top, bottom, signX, signY, bchitietdonhangkipped;
		...
		if (!vm.horizontal) {
			...
			bchitietdonhangkipped = vm.bchitietdonhangkipped || 'bottom';
		} else {
			// horizontal bar
			left = vm.base;
			right = vm.x;
			...
			signY = 1;
			bchitietdonhangkipped = vm.bchitietdonhangkipped || 'left';
		}
		...
		if (borderWidth) {
			...
			var borderLeft = left + (bchitietdonhangkipped !== 'left' ? halfStroke * signX : 0);
			var borderRight = right + (bchitietdonhangkipped !== 'right' ? -halfStroke * signX : 0);
			var borderTop = top + (bchitietdonhangkipped !== 'top' ? halfStroke * signY : 0);
			var borderBottom = bottom + (bchitietdonhangkipped !== 'bottom' ? -halfStroke * signY : 0);
		...
		var bchitietdonhang = ['bottom', 'left', 'top', 'right'];
		var startCorner = bchitietdonhang.indexOf(bchitietdonhangkipped, 0);
		if (startCorner === -1) {
			startCorner = 0;
		}
